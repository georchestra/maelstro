...
class GNSession:
    def __init__(self, apiUrl: str, user: str, pw: str):
        ...
        if r.status_code == 200:
            jj = r.json()
            print(
                f"Connected as {jj['name']} {jj['surname']} with profile {jj['profile']}"
            )
        else:
            r = self._session.get(
                f"{self._apiUrl}/me", headers={"Accept": "application/json"}
            )
    ...
    def getRecordIds(self):
        return [
            ...
            for i in self._session.post(
                f"{self._apiUrl}/search/records/_search",
                headers={"Accept": "application/json"},
                json={},
            ).json()["hits"]["hits"]
        ]
    ...
    def getRecordZip(self, uuid: str):
        r = self._session.get(
            f"{self._apiUrl}/records/{uuid}", headers={"Accept": "application/zip"}
        )
        if r.status_code == 200:
            return r.content
    ...
    def getRecordXml(self, uuid: str):
        r = self._session.get(
            f"{self._apiUrl}/records/{uuid}", headers={"Accept": "application/xml"}
        )
        if r.status_code == 200:
            return r.content
    ...
    def uploadRecordZip(self, zipFile: BinaryIO, overwrite: bool = True):
        return self._session.post(
            ...
            files={"file": ("file.zip", zipFile, "application/zip")},
            params={
    ...
    def uploadRecordXml(self, xmlFile: BinaryIO, overwrite=True):
        return self._session.post(
            ...
            files={"file": ("file.xml", xmlFile, "application/zip")},
            params={
                "metadataType": "METADATA",
                "uuidProcessing": "OVERWRITE" if overwrite else "GENERATEUUID",
            },
